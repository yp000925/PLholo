...
class PLPhaseNet_block(nn.Module):
    def __init__(self, alpha=4):
        ...
        self.denoiser = denoiser(1)

    # def FFT2D(self,x):
    #     return ifftshift(fft2(fftshift(x,dim = [1,2])),dim=[1,2])
    #
    # def IFFT2D(self,x):
    #     return ifftshift(ifft2(fftshift(x,dim=[1,2])),dim= [1,2])

    def batch_forward_proj(self,field_batch, otf3d_batch, intensity=True, scale = True):
        '''

        :param field_batch:  3d field for batch [B, 1, H, W]
        :param otf3d_batch: OTF3d for batch [B,1, H, W]
        :return: holo_batch [B, 1, H, W]
        '''
        [B, _, H, W] = field_batch.shape
        Fholo  =  torch.mul(fft2(field_batch),otf3d_batch)
        holo = ifft2(Fholo)
        if intensity:
            holo = torch.abs(holo)
            if scale:
